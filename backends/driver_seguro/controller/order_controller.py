...
def inserir_pedido(orderid, customerid, employeename, produtos=[]):
    ...
    sessao.execute(
        f"SELECT customerid FROM northwind.customers WHERE customerid = %s", (customerid)
    )
    cliente_existe = sessao.fetchall()

    sessao.execute(
        f"SELECT employeeid FROM northwind.employees WHERE firstname = %s", (employeename)
    )
    employee_tuple = sessao.fetchone()

    sessao.execute(
        f"SELECT orderid FROM northwind.orders WHERE orderid = %s", (orderid)
    )
    ...
    for produto in produtos:
        sessao.execute(
            f"SELECT unitprice FROM northwind.products WHERE productid = %s", (produto['productid'])
        )
